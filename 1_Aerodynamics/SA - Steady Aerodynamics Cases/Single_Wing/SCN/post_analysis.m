...
N = [6 12 24 48 96];
for i = 1:5
   eA8(i) = abs((CL-CLA8(i)))/CL*100;
   reA8(i) = abs((CLA8(end)-CLA8(i)))/CLA8(end)*100;
end

figure(5)
plot(log(N)/log(2),log(eA8)/log(),'--ro')
hold on
plot(log(N)/log(2),log(reA8),'-bs')
grid on
xlabel('N number of points in the mesh')
legend('relative error with theoretical CL','relative error with more refined mesh')
